...
async function loadWords() {
    ...
    const filePath = `../../${level}/${unit}/data3/words.txt`; // Monta o caminho do arquivo dinamicamente
...
// AJUSTADO: apenas horizontal (esquerda -> direita)
function placeWordInGrid(word) {
    const direction = { row: 0, col: 1 }; // apenas horizontal (E → D)
    let row, col;

    // tenta várias vezes achar um ponto inicial válido
    ...
    return false; // não conseguiu posicionar
}
...
// AJUSTADO: seleção apenas horizontal, contígua, esquerda -> direita
function handleCanvasClick(event) {
    ...
    if (selectedCells.length === 0) {
        ...
        // Regras: mesma linha E contígua à direita (coluna + 1)
        const sameRow = row === lastCell.row;
        const nextRight = col === lastCell.col + 1;

        if (sameRow && nextRight) {
            selectedCells.push({ row, col });
        } else {
            // qualquer coisa fora da regra reinicia a seleção a partir desta célula
            selectedCells = [{ row, col }];
        }
    }
